...
def getMessageAndInsertDB(latest_mess_id,all_message,cnxpool): #for table message
...
def getOpenTradesAndInsertDB(cnxpool):
    _lock = threading.Lock()
    _lock.acquire()
...
def dbInsert_0(latest_mess_id,trade_id_dict,response_dict_list,cnxpool):
    try:
        connection = cnxpool.get_connection()
...
def sendTradesAndInsertDB(trades_dict,latest_message_id,cnxpool):
    ...
    if len(trade_id_dict) > 0:
        t = threading.Thread(name="dbInsert",target=dbInsert_0,args = (latest_message_id,trade_id_dict,trade_sender_response,cnxpool,))
        t.daemon = True
        t.start()
        t.join()
...
def getRecentTradesAndSendEmail(message,cnxpool):
        """
        :param subject: email subject
        :param message: Body content of the email (string), can be HTML/CSS or plain text
        :param from_email: Email address from where the email is sent
